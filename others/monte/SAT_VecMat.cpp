...
// [jmm220615] Dóna error: sembla que té a veure amb que versions velles
// de g++ no suportaven totes les coses
...
Matrix& Matrix::resize(int dim1, int dim2) {
  ...
  j_max = ((dim2 < m) ? dim2 : m);
  for (i = 0; i < i_max; i++)
    for (j = 0; j < j_max; j++) M_new[i][j] = M[i][j];
  for (i = i_max; i < dim1; i++)
    for (j = 0; j < dim2; j++) M_new[i][j] = 0.0;
  for (i = 0; i < i_max; i++)
    for (j = j_max; j < dim2; j++) M_new[i][j] = 0.0;
  // Dispose unused memory
